...
import { Images, Messages, NewOrder, Order, UpdatedOrder } from '@shared/models/order.interface';
...
export default class EditsComponent {
  ...
  order = signal<Order>(this.entity_blank);
  ...
  async createOrder() {
    ...
    const entity: UpdatedOrder = {
      id: this.order()?.id,
      // model_composition: this.order()?.model_composition,
      // model_title: this.order()?.model_title,
      // model_image: this.order()?.model_image,
      // model_price: this.order()?.model_price,
      // order_state: "created",
      messages: messages,
      images: images,
      // video_rendered_url: '',
